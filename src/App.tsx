...
function App() {
  ...
  const saveDraft = () => {
    ...
    setNotice(`${item.id} saved as draft.`)
  }
    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <div className="content-wrap">
          {page === 'new' && <FormPage rows={rows} total={total} locations={locationOptions} modes={modeOptions} employee={employee} setEmployee={setEmployee} department={department} setDepartment={setDepartment} position={position} setPosition={setPosition} purpose={purpose} setPurpose={setPurpose} dateFiled={dateFiled} setDateFiled={setDateFiled} updateRow={updateRow} addRow={addRow} removeRow={removeRow} saveDraft={saveDraft} notice={notice} />}
...
function FormPage(props: { rows: Row[]; total: number; locations: string[]; modes: string[]; employee: string; setEmployee: (v: string) => void; department: string; setDepartment: (v: string) => void; position: string; setPosition: (v: string) => void; purpose: string; setPurpose: (v: string) => void; dateFiled: string; setDateFiled: (v: string) => void; updateRow: (id: number, key: keyof Row, value: string) => void; addRow: () => void; removeRow: (id: number) => void; saveDraft: () => void; notice: string }) {
  const { rows, total, locations, modes, employee, setEmployee, department, setDepartment, position, setPosition, purpose, setPurpose, dateFiled, setDateFiled, updateRow, addRow, removeRow, saveDraft, notice } = props
  return <>
    <div className="page-heading"><div><div className="eyebrow">TRANSPORT REQUESTS / CREATE</div><h1>New transportation request</h1><p>Prepare a request for review. Fare rates are filled automatically from your route.</p></div><div className="heading-actions"><button className="secondary-button" onClick={() => window.print()}><Printer size={16} /> Print preview</button><button className="primary-button" onClick={saveDraft}><Check size={16} /> Save draft</button></div></div>
    ...
    <section className="form-paper">
      ...
      <div className="employee-grid">
        <Field label="Name of Employee" value={employee} onChange={setEmployee} required />
        ...
        <Field label="Department" value={department} onChange={setDepartment} options={['Operations', 'Finance', 'Administration', 'Human Resources']} />
...
function Field({ label, value, onChange, type = 'text', required = false, options }: { label: string; value: string; onChange: (v: string) => void; type?: string; required?: boolean; options?: string[] }) { return <div className="field"><label>{label} {required && <span className="required">*</span>}</label>{options ? <Select value={value} options={options} onChange={onChange} /> : <input type={type} value={value} onChange={(e) => onChange(e.target.value)} />}</div> }
